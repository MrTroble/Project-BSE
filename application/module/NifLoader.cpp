...
namespace tge::nif
{
		...
		NifModule::load(const std::string& name, const tge::graphics::NodeTransform& baseTransform, void* shaderPipe) const
	{
		...
		for (auto shape : shapes)
		{
			...
			{
				const auto pipe = sha->compile(
					{ { ShaderType::VERTEX, vertexFile, cacheString },
		...
		for (const auto shape : shapes)
		{
			...
			if (bishape)
			{
				...
				if (shaderData)
				{
					...
					const BindingInfo albedoBinding{ 1,
													 nodeInfo.bindingID,
													 BindingType::Texture,
													 { albedoID, samplerID } };
					const BindingInfo normalBinding{ 4,
													 nodeInfo.bindingID,
													 BindingType::Texture,
													 { normalID, samplerID } };
