...
class TGAppGUI : public tge::gui::GUIModule {
 public:
  ...
  void renderGUI() override {
    if (api != nullptr) shaderUI.renderGUI(api->getShaderAPI());
    if (ImGui::Begin("test")) {
      const bool lightBool =
          ImGui::SliderFloat3("Light", (float*)&light.pos, -10.0f, 10.0f);
      const bool intensityBool = ImGui::SliderFloat(
          "Intesity", (float*)&light.intensity, -10.0f, 10.0f);
      const bool valueBool =
          ImGui::SliderFloat("Min Ratio", (float*)&light.minRatio, 0.0f, 1.0f);
...
#ifdef DEBUG
      const auto sceneGraph = ImGui::BeginChild("Scene Graph");
      if (ggm != nullptr && sceneGraph) {
        ...
        if (ImGui::InputText("Search", search, 100)) {
          ...
          if (search[1] != 0) {
            for (int i = values.size() - 1; i >= 0; i--) {
              ...
              const auto& nameChild = values[index] == nullptr
        ...
        for (size_t i = 0; i < values.size(); i++) {
          ...
          const auto& name = values[index1] == nullptr
                                 ? "Unknown #" + std::to_string(index1)
                                 : values[index1]->name;
          if (extended.contains(index1)) ImGui::SetNextItemOpen(true);
            ...
            while (!nextProcess.empty()) {
              ...
              if (ImGui::TreeNode(nameChild.c_str())) {
                ...
                if (ImGui::InputFloat3("Translation",
                                       &nodeTrans.translation[0]))
                  marker[index] = 1;
                if (ImGui::InputFloat3("Scale", &nodeTrans.scale[0]))
                  marker[index] = 1;
                auto euler = glm::eulerAngles(nodeTrans.rotation);
                if (ImGui::SliderFloat3("Rotation", &euler[0], 0,
                                        2 * std::numbers::pi)) {
